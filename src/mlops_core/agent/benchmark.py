...
reporting a share as a percentage is not.
...
from mlops_core.agent.prompts import ROUTER_VERSION, ROUTES, SQL_VERSION, Route
...
def same_answer(expected: QueryResult, got: QueryResult) -> bool:
    ...
    column of `got` with the same values, and the rows the same, in any order."""
    ...
    with mlflow.start_run(run_name=generator.split("@")[0]) as run:
        ...
        params: dict[str, Any] = {
            ...
            **{f"option_{k}": v for k, v in GENERATOR_OPTIONS.items()},
            ...
            "case_files_sha256": _digest(case_files),
        }
